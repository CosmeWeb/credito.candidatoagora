...
class Slim {
    ...
    private static function parseInput($value) {
        ...
        if(isset($data->input))
        {
            ...
            {
                $filename = $_FILES[$data->input->field]['tmp_name'][0];
                if($filename)
                {
                    ...
                    $inputData = Slim::getBase64Data($inputData);*/
                }
            }
            $input = array(
                'data' => $inputData,
    ...
    ##############################################################################################
    public static function GetFile($inputName)
    {
        $width = 0;
        $height = 0;
        $type = $_FILES[$inputName]['type'][0];
        $tipos = array('image/jpeg', 'image/jpg', 'image/gif', 'image/png');;
        if(in_array(strtolower($type), $tipos) === false)
            return false;
        $filename = $_FILES[$inputName]['tmp_name'][0];
        if(!empty($filename))
        {
