...
?>
    <div class="col-lg-12">
        <div class="panel panel-blue">
            <div class="panel-heading"><?php echo __("Filtro de Newsletter") ?></div>
            <div class="panel-body pan">
                <form id="frmnewsletter" action="<?php echo site_url('newsletter/listar/'); ?>" class="horizontal-form" method="GET">
                    <div class="form-body pal">
                        ...
                        <div class="row" id="divImportar" style="display: none; background-color: #e0f3e3; padding: 20px 0px;">
                            <div class="form-group">
                                <label for="subarea" class="col-md-3 control-label text-right"><?php echo __("Importar arquivo xls de newsletters"); ?> </label>
                                <div class="col-md-5">
                                    <div class="input-icon">
                                        <i class="fa fa-table"></i>
                                        <input id="filenewsletter" name="filenewsletter" type="file" value="" class="form-control">
                                    </div>
                                </div>
                                <div class="col-md-4">
                                    ...
                                    <a href="javascript:;" class="btn pull-right btn-exportar" style="color: #FF0000;" onclick="FecharImportarNewsletter();">
                        ...
                        <div class="row" id="divExportar" style="display: none;">
                            <div class="col-lg-12 mtm mbm">
                                <a href="javascript:;" class="btn pull-right btn-exportar" onclick="FecharExportarNewsletter();">
                                    <i class="fa fa-close"></i>
                                </a>
                    ...
                    <div class="form-actions text-right pal">
                        ...
                        <button type="button" class="btn btn-green" onclick="AdicionarNovo('<?php echo site_url('newsletter/editar/'); ?>')"><i class="fa fa-plus"></i> <?php echo __("Adicionar Newsletter") ?></button>
                        <button type="button" class="btn btn-blue" onclick="AbrirImportarNewsletter()"><i class="fa fa fa-sign-in"></i> <?php echo __("Importar Newsletter") ?></button>
                        <button type="button" class="btn btn-blue" onclick="ExportarNewsletter();"><i class="fa fa-sign-out"></i> <?php echo __("Exportar Newsletter") ?></button>
                    </div>
                </form>
            </div>
        </div>
	...
	<div class="col-lg-12">
		<div class="portlet box">
			<div class="portlet-header">
				<div class="caption"><?php echo __("Lista de Newsletter") ?></div>
			</div>
			<div class="portlet-body">
				<div class="row mbm">
					<div class="col-lg-12">
						<div class="table-responsive">
							<table id="newsletter_tabela" class="table table-hover table-bordered table-advanced display">
								<thead>
								<tr>									
                                    <th style="width: 8%;"><?php echo __("ID"); ?></th>
                                    <th style="width: 42%;"><?php echo __("Email"); ?></th>
                                    <th style="width: 20%;"><?php echo __("IP"); ?></th>
                                    <th style="width: 20%;"><?php echo __("Cadastrado em"); ?></th>
...
<script>
    ...
    function confirmar(obj)
    {
        ...
        confirm("<?php echo __("Tem certeza que deseja deletar definitivamente a newsletter?<br /><b>Atenção: Ao confirmar não será possível desfazer essa ação.</b>"); ?>", "<?php echo __("Confirme"); ?>", function(confirmed) {
            if (confirmed)
            {
                window.location = url;
            }
    ...
    {
        var url = GetUrlAcao("newsletter","exportarnewsletter");
        ...
        $.ajax({
            ...
            error: function(XHR, textStatus, errorThrown){
                $('#divloding').slideUp("slow");
                msn = "<?php echo __("Falha na de verificação de newsletter");?>";
                alert(msn);
            }
        });
    }
    function FecharExportarNewsletter()
    {
        $('#filenewsletter').val("");
        $('#divExportar').slideUp("slow");
    }
    function FecharImportarNewsletter()
    {
        $('#divImportar').slideUp("slow");
        document.getElementById('filenewsletter').files = null;
        $('filenewsletter').val('');
    }
    function AbrirImportarNewsletter()
    {
        $('#divImportar').slideDown("slow");
    }
    function EnviarExcel(){
        var property = document.getElementById('filenewsletter').files[0];
        if(Vazio(property))
        {
            alert("<?php echo __("Você deve selecionar o arquivo com os dados de importação de Newsletters");?>");
            return;
        }
        var file_name = property.name;
        var file_extension = file_name.split('.').pop().toLowerCase();
        var urlfile = GetUrlAcao("newsletter","enviarexcel");
        ...
        $.ajax({
            ...
            success:function(data){
                if(data.sucesso)
                {
                    ...
                    msn = "<?php echo __("Arquivo foi enviado com sucesso.<br/>Aguarde a finalização do processo de importação de Newsletter.");?>";
                    MsnSucesso(titulo, msn);
    ...
    function ImportacaoExcel(file, posicao, total){
        ...
        var url = GetUrlAcao("newsletter","importacaoexcel");
    ...
    $(document).ready(function() {
        var el_datatable = $('#newsletter_tabela').DataTable({
            "language":{ "url": "//cdn.datatables.net/plug-ins/1.10.12/i18n/Portuguese-Brasil.json" },
            "lengthMenu": [[10, 25, 50, 100, -1], [10, 25, 50, 100, "All"]],
            "processing": true,
            "serverSide": true,
            "ajax": {
                "url": GetDominio('index.php/newsletter/listatabela/'),
                "data": function ( d ) {
                    return $.extend( {}, d, {"FILTRO": $('#frmnewsletter').serializeArray()} );
                },
                "type": 'post',
                "dataType": 'json',
            },
            columns: [
                {"data": "idnewsletter", render: $.fn.dataTable.render.number(',', '.', '')},
                ...
                {"data": "idnewsletter",
                    "render": function ( data, type, row, meta ) {
                        var url = GetDominio('index.php/newsletter/editar/'+data);
                        var link = '<a href="'+url+'" class="btn-tab-editar" title="Editar newsletter"><i class="fa fa-edit"></i></a>';
                        url = GetDominio('index.php/newsletter/excluir/'+data);
                        link += '<a href="javascript:;" class="btn-tab-excluir" title="Excluir newsletter" onclick="confirmar(this)" data-url="'+url+'"><i class="ion ion-android-trash"></i></a>';
                        return link;
                    }
                }
            ],
        ...
        el_datatable.on( 'draw', function () {
            $('#newsletter_tabela_filter').css('display','none');
        } );
