...
class MY_Model extends CI_Model
{
	...
	################################################################################################################
	public static function GeraOpcoesArray($valor = "", $vetor = [], $primeiro = false)
	{
		$lista = "";
		if(self::$exibirprimeiro)
		{
	...
	public function TotalRegistro($filtro = "", $sql = "", $limparsql = false)
	{
		...
		try
		{
			...
			if(!empty($filtro))
			{
				$pos = stripos($filtro, ' GROUP BY');
				if ($pos !== false)
				{
					$filtro = trim(stristr($filtro, ' GROUP BY', true));
				}
				elseif(stripos($filtro, ' ORDER BY') !== false)
				{
