...
class Area extends CI_Controller {
	...
	{
		$area = GetModelo('area');
		$linkRetorno = GetReferencia('area/listar');
		if( !empty($id))
		{
			$area->idarea = $id;
			if(!$area->Load())
			{
				$data = array('act'=>__("Erro"),'titulo'=>__("Àrea não foi localizado."));
				$this->gestao->GetView('layout/erro', $data );
				return;
			}
			$this->gestao->SetBreadcrumbs("Editar",site_url('area/listar'), 'Área');
			$data['obj'] = $area;
		}
		else
		{
			$this->gestao->SetBreadcrumbs("Novo cadastro",site_url('area/listar'), 'Área');
			$data['obj'] = $area;
		}
		$this->load->library('form_validation');
		
		$this->form_validation->set_rules( 'area', __("área"), 'required' );

		if( $this->form_validation->run() === FALSE )
		{
			$this->gestao->GetView('area/editar', $data);
		}
		else
		{
			$post = $this->gestao->Request($area->GetDefault());
			$area->Carregar($post);
			$area->Ajustar(true);
			if(!$area->Salvar())
			{
				$data = array('act'=>"Erro",'titulo'=>__("Não foi possível salvar os dados da área."));
				$this->gestao->GetView('layout/erro', $data);
				return;
			}
			if(!empty($id))
			{
				message( 'success', 'msg_flash', __("Área foi editada com sucesso!") );
			}
			else
			{
				message( 'success', 'msg_flash', __("Área foi adicionada com sucesso!") );
			}
	...
	public function excluir($id = 0)
	{
		...
		if(!empty($id))
		{
			...
			if($area->Apagar())
			{
				message( 'success', 'msg_flash', __("Área foi excluida com sucesso!") );
			}
			else
			{
				message( 'warning', 'msg_flash', __("Não foi possível excluir a área.") );
			}
		}
		else
		{
			message( 'warning', 'msg_flash', __("Não foi possível localizar a área.") );
		}
	...
	{
		$this->gestao->SetBreadcrumbs("Lista de Área");
		self::LimparArea();
		$data['obj'] = GetModelo('area');
		$this->gestao->GetView('area/listar', $data);
	}
	#######################################################################################################
	...
	#######################################################################################################
	public static function LimparArea()
	{
		$obj = GetModelo('Area');
		$path = $obj->SetDominio();
		if(!is_dir($path))
	...
	public function exportararea()
	{
		...
		{
			$dados['sucesso'] = false;
			$dados['erro'] = __("Erro ao carregar a classe da área.");
		}
		RetornaJSON($dados);
	}
	#######################################################################################################
	...
	public function importacaoexcel()
	{
		...
		{
			$dados['sucesso'] = false;
			$dados['erro'] = __("Erro ao localizar o registro de importação de área.");
		}
		RetornaJSON($dados);
	}
}
