...
if(!function_exists('controller'))
{
    ...
    {
        $filename = realpath(BASEPATH. '/../controllers/'.$name.'.php');
...
function Acesso($classeExclude = null, $metodoExclude = null)
{
	...
	if( !isset( $session['idcolaborador'] ) or !isset( $session['email'] ) or !isset( $session['nome'] ) ):
...
#####################################################################################################
function GetAcesso($chave = "idcolaborador")
{
	$CI = &get_instance();
	$session = $CI->session->all_userdata();
...
#####################################################################################################
function TemAcessoDashboard()
{
	$acesso = GetAcesso("visualizadashboard");
	if($acesso == "Sim")
		return true;
	else
		return false;
...
function GetDomino($url = "")
{
	...
	if (empty($config['dominio']))
	{
		if (isset($_SERVER['SERVER_ADDR']))
		{
			...
			$dominio = (is_https() ? 'https' : 'http').'://'.$server_addr
