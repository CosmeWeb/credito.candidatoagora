                ...
                <form id="frmarea" action="<?php echo site_url('area/listar/'); ?>" class="horizontal-form" method="GET">
                    <div class="form-body pal">
                        ...
                        <div class="row" id="divImportar" style="display: none; background-color: #e0f3e3; padding: 20px 0px;">
                            <div class="form-group">
                                <label for="subarea" class="col-md-3 control-label text-right"><?php echo __("Importar arquivo xls de áreas"); ?> </label>
                                <div class="col-md-5">
                        ...
                        <div class="row" id="divBarra" style="display: none;">
                            <div class="col-lg-12 mtm">
                                <h5> <?php echo __("Importação de Area"); ?></h5>
                    ...
                    <div class="form-actions text-right pal">
                        ...
                        <button type="button" class="btn btn-green" onclick="AdicionarNovo('<?php echo site_url('area/editar/'); ?>')"><i class="fa fa-plus"></i> <?php echo __("Adicionar Área") ?></button>
                        <button type="button" class="btn btn-blue" onclick="AbrirImportarArea()"><i class="fa fa fa-sign-in"></i> <?php echo __("Importar Área") ?></button>
                        <button type="button" class="btn btn-blue" onclick="ExportarArea();"><i class="fa fa-sign-out"></i> <?php echo __("Exportar Área") ?></button>
                    </div>
                </form>
            </div>
        </div>
						...
						<div class="table-responsive">
							<table id="area_tabela" class="table table-hover table-bordered table-advanced display">
								...
								<tr>									
									...
									<th style="width: 84%;"><?php echo __("Área"); ?></th>
...
<script>
    function confirmar(obj)
    {
        ...
        confirm("<?php echo __("Tem certeza que deseja deletar definitivamente a área?<br /><b>Atenção: Ao confirmar não será possível desfazer essa ação.</b>"); ?>", "<?php echo __("Confirme"); ?>", function(confirmed) {
            if (confirmed)
            {
                window.location = url;
            }
    ...
    {
        var url = GetUrlAcao("area","exportararea");
        var data = $("#frmarea").serialize();
        $.ajax({
            url: url,
            method:'POST',
            data:data,
            ...
            error: function(XHR, textStatus, errorThrown){
                $('#divloding').slideUp("slow");
                msn = "<?php echo __("Falha na de verificação de área");?>";
                alert(msn);
            }
        });
    }
    ...
    function EnviarExcel(){
        ...
        if(Vazio(property))
        {
            alert("<?php echo __("Você deve selecionar o arquivo com os dados de importação de Áreas");?>");
            return;
        }
        var file_name = property.name;
        var file_extension = file_name.split('.').pop().toLowerCase();
        var urlfile = GetUrlAcao("area","enviarexcel");
        ...
        $.ajax({
            ...
            success:function(data){
                if(data.sucesso)
                {
                    ...
                    msn = "<?php echo __("Arquivo foi enviado com sucesso.<br/>Aguarde a finalização do processo de importação de Área.");?>";
                    MsnSucesso(titulo, msn);
    ...
    function ImportacaoExcel(file, posicao, total){
        ...
        var url = GetUrlAcao("area","importacaoexcel");
    ...
    $(document).ready(function() {
        var el_datatable = $('#area_tabela').DataTable({
            ...
            "serverSide": true,
            "ajax": {
                "url": GetDominio('index.php/area/listatabela/'),
                "data": function ( d ) {
                    return $.extend( {}, d, {"FILTRO": $('#frmarea').serializeArray()} );
                },
                "type": 'post',
                "dataType": 'json',
            },
            columns: [
                ...
                {"data": "idarea",
                    "render": function ( data, type, row, meta ) {
                        var url = GetDominio('index.php/area/editar/'+data);
                        var link = '<a href="'+url+'" class="btn-tab-editar" title="Editar área"><i class="fa fa-edit"></i></a>';
                        url = GetDominio('index.php/area/excluir/'+data);
                        link += '<a href="javascript:;" class="btn-tab-excluir" title="Excluir área" onclick="confirmar(this)" data-url="'+url+'"><i class="ion ion-android-trash"></i></a>';
                        return link;
                    }
                }
            ],
