...
?>
    <div class="col-lg-12">
        <div class="panel panel-blue">
            ...
            <?php echo __("Filtro de Curso") ?>
                <div class="tools">
                    <i class="fa fa-chevron-down" onclick="ExibePainel(this,'#painelcurso')"></i>
                </div>
            </div>
            <div id="painelcurso" class="panel-body pan">            
                <form id="frmcurso" action="<?php echo site_url('curso/listar/'); ?>" class="horizontal-form" method="GET">
                    <div class="form-body pal">
                        ...
                        <div class="row" id="divImportar" style="display: none; background-color: #e0f3e3; padding: 20px 0px;">
                            <div class="form-group">
                                <label for="subarea" class="col-md-3 control-label text-right"><?php echo __("Importar arquivo xls de cursos"); ?> </label>
                                <div class="col-md-5">
                                    <div class="input-icon">
                                        <i class="fa fa-table"></i>
                                        <input id="filecurso" name="filecurso" type="file" value="" class="form-control">
                                    </div>
                                </div>
                                <div class="col-md-4">
                                    ...
                                    <a href="javascript:;" class="btn pull-right btn-exportar" style="color: #FF0000;" onclick="FecharImportarCurso();">
                        ...
                        <div class="row" id="divBarra" style="display: none;">
                            <div class="col-lg-12 mtm">
                                <h5> <?php echo __("Importação de Curso"); ?></h5>
                        ...
                        <div class="row" id="divExportar" style="display: none;">
                            <div class="col-lg-12 mtm mbm">
                                <a href="javascript:;" class="btn pull-right btn-exportar" onclick="FecharExportarCurso();">
                                    <i class="fa fa-close"></i>
                                </a>
                    ...
                    <div class="form-actions text-right pal">
                        ...
                        <button type="button" class="btn btn-green" onclick="AdicionarNovo('<?php echo site_url('curso/editar/'); ?>')"><i class="fa fa-plus"></i> <?php echo __("Adicionar Curso") ?></button>
                        <button type="button" class="btn btn-blue" onclick="AbrirImportarCurso()"><i class="fa fa fa-sign-in"></i> <?php echo __("Importar Curso") ?></button>
                        <button type="button" class="btn btn-blue" onclick="ExportarCurso();"><i class="fa fa-sign-out"></i> <?php echo __("Exportar Curso") ?></button>
                    </div>
                </form>
            </div>
        </div>
	...
	<div class="col-lg-12">
		<div class="portlet box">
			<div class="portlet-header">
				<div class="caption"><?php echo __("Lista de Curso") ?></div>
			</div>
			<div class="portlet-body">
				<div class="row mbm">
					<div class="col-lg-12">
						<div class="table-responsive">
							<table id="curso_tabela" class="table table-hover table-bordered table-advanced display">
								<thead>
								<tr>
									<th style="width: 8%;"><?php echo __("ID"); ?></th>
									<th style="width: 84%;"><?php echo __("Curso"); ?></th>
...
<script>
    function confirmar(obj)
    {
        ...
        let msn = "<?php echo __("Tem certeza que deseja deletar definitivamente o curso?<br /><b>Atenção: Ao confirmar não será possível desfazer essa ação.</b>"); ?>";
    ...
    function ExportarCurso(file = "", posicao = 0, total = 0)
    {
        let url = GetUrlAcao("curso","exportarcurso");
        let data = {
            "file":file,
            "posicao": posicao,
            "total": total,
        };
        data = $.extend( {}, data, {"FILTRO": $("#frmcurso").serializeArray()} );
        $.ajax({
            url: url,
            method:'POST',
            data:data,
            ...
            success:function(data){
                if(data.sucesso)
                {
                    ...
                    {
                        ExportarCurso(data.file, data.posicao, data.total);
                    }
                }
                else
                {
            ...
            error: function(XHR, textStatus, errorThrown){
                let msn = "<?php echo __("Falha na de verificação de curso");?>";
                $('#divloding').slideUp("slow");
                $('#divBarra').slideUp("slow");
                alert(msn);
            }
    ...
    function FecharExportarCurso()
    {
        $('#filecurso').val("");
        $('#divExportar').slideUp("slow");
    }
    function FecharImportarCurso()
    {
        $('#divImportar').slideUp("slow");
        document.getElementById('filecurso').files = null;
        $('filecurso').val('');
    }
    function AbrirImportarCurso()
    {
        $('#divImportar').slideDown("slow");
    }
    function EnviarExcel(){
        let property = document.getElementById('filecurso').files[0];
        if(Vazio(property))
        {
            alert("<?php echo __("Você deve selecionar o arquivo com os dados de importação de Cursos");?>");
            return;
        }
        let file_name = property.name;
        let file_extension = file_name.split('.').pop().toLowerCase();
        let urlfile = GetUrlAcao("curso","enviarexcel");
        ...
        $.ajax({
            ...
            success:function(data){
                if(data.sucesso)
                {
                    ...
                    msn = "<?php echo __("Arquivo foi enviado com sucesso.<br/>Aguarde a finalização do processo de importação de Curso.");?>";
                    MsnSucesso(titulo, msn);
    ...
    function ImportacaoExcel(file, posicao, total){
        ...
        let url = GetUrlAcao("curso","importacaoexcel");
    ...
    $(document).ready(function() {
        var el_datatable = $('#curso_tabela').DataTable({
            "language":{ "url": CDN_Url('datatables') },
            "lengthMenu": [[10, 25, 50, 100, -1], [10, 25, 50, 100, "All"]],
            "processing": true,
            "serverSide": true,
            "ajax": {
                "url": GetDominio('index.php/curso/listatabela/'),
                "data": function ( d ) {
                    return $.extend( {}, d, {"FILTRO": $('#frmcurso').serializeArray()} );
                },
                "type": 'post',
                "dataType": 'json',
            },
            columns: [
                {"data": "idcurso", render: $.fn.dataTable.render.number(',', '.', '')},            
				{'data': 'curso'},
                {"data": "idcurso",
                    "render": function ( data, type, row, meta ) {
                        let url = GetDominio('index.php/curso/editar/'+data);
                        let link = '<a href="'+url+'" class="btn-tab-editar" title="Editar curso"><i class="fa fa-edit"></i></a>';
                        url = GetDominio('index.php/curso/excluir/'+data);
                        link += '<a href="javascript:;" class="btn-tab-excluir" title="Excluir curso" onclick="confirmar(this)" data-url="'+url+'"><i class="ion ion-android-trash"></i></a>';
                        return link;
                    }
                }
            ],
        ...
        el_datatable.on( 'draw', function () {
            $('#curso_tabela_filter').css('display','none');
        } );
    });
</script>