						...
						<div class="table-responsive">
							<table id="rastreio_tabela" class="table table-hover table-bordered table-advanced display">
								...
								<tr>									
									...
									<th style="width: 10%;"><?php echo __("Código"); ?></th>
									<th style="width: 30%;"><?php echo __("Descrição"); ?></th>
...
<script>
    ...
    function confirmar(obj)
    {
        ...
        let msn = "<?php echo __("Tem certeza que deseja deletar definitivamente o rastreio?<br /><b>Atenção: Ao confirmar não será possível desfazer essa ação.</b>"); ?>";
    ...
    function ExportarPDFRastreio()
    {
        ...
        if(Vazio(idcliente))
        {
            alert('Você deve selecionar o cliente para emitir relatório em PDF');
            return;
        }
        form.get(0).submit();
    }
    function ExportarRastreio(tipo = "", file = "", posicao = 0, total = 0)
    {
        ...
        $.ajax({
            ...
            error: function(XHR, textStatus, errorThrown){
                let msn = "<?php echo __("Falha na de verificação de rastreio");?>";
                $('#divloding').slideUp("slow");
                $('#divBarra').slideUp("slow");
                alert(msn);
            }
    ...
    function EnviarExcel(){
        ...
        if(Vazio(property))
        {
            alert("<?php echo __("Você deve selecionar o arquivo com os dados de importação de Rastreios");?>");
            return;
        }
        ...
        $.ajax({
            ...
            success:function(data){
                if(data.sucesso)
                {
                    ...
                    msn = "<?php echo __("Arquivo foi enviado com sucesso.<br/>Aguarde a finalização do processo de importação de Rastreio.");?>";
                    MsnSucesso(titulo, msn);
