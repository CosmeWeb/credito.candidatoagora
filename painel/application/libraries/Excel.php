...
class Excel {
	...
	var $NomeColuna = array();
	var $StyleClasse = "";
	var $LaguraTabela = 920;
	/**
	...
	protected function _read($sFileName) {
		...
		//$aux = substr($this->data,$this->numBigBlockDepotBlocks, 2000);
			...
			switch ($code) {
				case self::Spreadsheet_Excel_Reader_Type_SST:
					...
					for ($i = 0; $i < $uniqueStrings; $i++)
					{
						...
						$retstr = ($asciiEncoding) ? $this->_encodeUTF8($retstr ) : $this->_encodeUTF16($retstr);
						if ($richString)
						{
	...
	################################################################################################################
	protected function &_encodeUTF8(&$string)
	{
		return utf8_encode($string);
	}
	################################################################################################################
	public function Importar($posicao = 0, $limite = false)
	{
		if($this->SuperClasse)
		{
			$this->GetLinhas();
			...
			$IsaAjustarImportar = method_exists($this->SuperClasse,'ajustarImportar');
			$IsaValidarImportacao = method_exists($this->SuperClasse,'ValidarImportacao');
			for($linha = $posicao; $linha < $limite && $linha < $total; $linha++)
			{
				$ValoresLinha = $this->getLinha($linha);
				if(!$ValoresLinha)
					continue;
				if($IsaAjustarImportar)
				{
					$ValoresLinha = $this->SuperClasse->ajustarImportar($ValoresLinha);
				}
				$this->SuperClasse->Carregar( $ValoresLinha );
				if($IsaValidarImportacao)
				{
					if(!$this->SuperClasse->ValidarImportacao())
						continue;
				}
				$this->SuperClasse->Importar();
			}
			return true;
		}
		return false;
	...
	################################################################################################################
	public function getLinha($indece = 0)
	{
		if(empty($this->Linhas[$indece]))
			$this->GetLinhas();
		return $this->Linhas[$indece];
	}
	################################################################################################################
	...
	{//return true;
		if($this->SuperClasse)
		{
			...
			if(method_exists($this->SuperClasse,'ValidarImportacao') )
			{
				$classname = get_class($this->SuperClasse);
				foreach ($this->Linhas as $Coluna)
				{
					$teste = call_user_func(array($classname,"ValidarImportacao"),$Coluna);
					if(!$teste)
					{
						self::$erro = $this->SuperClasse->GetErro()."\n";
						self::$erro .= " Planilha não foi importada.";
						return false;
					}
				}
			}
	...
	################################################################################################################
	public function GetLinhas($todas = false)
	{
		if(is_array($this->Linhas))
			return true;
		$this->Linhas = false;
		$this->Colunas = 0;
		foreach ($this->sheets as &$Planilha)
		{
			unset($Planilha['cellsInfo']);
			if($this->Colunas < $Planilha['numCols'])
				$this->Colunas = $Planilha['numCols'];
			if( ($Planilha['numCols'] == 0) && ($Planilha['numRows'] ==0) )
				continue;
			$primeiraLinha = !$todas;
			foreach ($Planilha['cells'] as $Coluna )
			{
				if($primeiraLinha)
				{
					$primeiraLinha = false;
				...
				$r = false;
				foreach($this->NomeColuna as $key=>$valor)
				{
					if(empty($Coluna[$key]))
						$r[$valor] = "";
					else 
			...
			$todas = false;
			unset($Planilha['cells']);
		}
		unset($this->sheets, $this->data, $this->sst, $this->numberFormats, $this->boundsheets,
		$this->dateFormats, $this->formatRecords, $this->bigBlockChain, $this->smallBlockChain,
		$this->entry, $this->props);
		return true;
	}
	################################################################################################################
	public function ExibePlanilha()
	{
		$this->GetLinhas(true);
		if(!is_array($this->Linhas))
		{
			$Tabela = '<div class="'.$this->StyleClasse.'" id="divexcel" style="width: '.$this->LaguraTabela.'px;"><table border="0" cellspacing="0" cellpadding="0"  >';
			$Tabela .= '<tr ><td class="nometabela"" >Exibeir dados da planilha</td></tr>';
			$Tabela .= '<tr ><td  class="TDmensagem">Planilha vazia</td></tr>';
			$Tabela .= '<tr ><td class="TDmensagem">&nbsp;</td></tr>';
			$Tabela .= '</table></div>';
			return $Tabela;
		}
		$Linha = count($this->Linhas);
		$Tabela = '<div class="'.$this->StyleClasse.'" id="divexcel" style="width: '.$this->LaguraTabela.'px;"><table border="0" cellspacing="0" cellpadding="0"  >';
		$Tabela .= '<tr ><td class="nometabela" colspan="'.$this->Colunas.'" > colunas:'.$this->Colunas.'  linhas: '.$Linha.'</td></tr>';
		$Tabela .= '<tr ><td  class="TDmensagem" colspan="'.$this->Colunas.'" >Ordem em que as colunas devem est&aacute;</td></tr>';
		$Tabela .= '<tr >';
		foreach ($this->NomeColuna as $Valor)
		{
			$Tabela .= '<td  class="TDcabecario">'.$Valor.'</td>';
		}
		$Tabela .= '</tr >';
		$Tabela .= '<tr ><td class="TDmensagem" colspan="'.$this->Colunas.'">Ordem em que as colunas est&atilde;o</td></tr>';
		$prilinha = true;
		foreach ($this->Linhas as $Coluna )
		{
			$Tabela .= '<tr >';
			foreach ($Coluna as $Key=>$Valor )
			{
				if($prilinha)
					$Tabela .= '<td class="TDcabecario">'.$Valor.'</td>';
				else
					$Tabela .= '<td class="TDvalor">'.$Valor.'</td>';
			}
			$Diferenca = count($this->NomeColuna)-count($Coluna);
			for($i = 0; $i < $Diferenca; $i++)
			{
				if($prilinha)
					$Tabela .= '<td class="TDcabecario">&nbsp;</td>';
				else
					$Tabela .= '<td class="TDvalor">&nbsp;</td>';
			}
			$Tabela .= '</tr >';
			if($prilinha)
				$prilinha = false;
	...
	{
		if ( ( $ws = $this->sheets[$worksheetIndex] ) === false )
		{
			return 0;
		}
		$numRows = $ws['numRows'];
	...
	################################################################################################################
	public function TotaldeColunas($worksheetIndex = 0)
	{
		if ( ( $ws = $this->sheets[$worksheetIndex] ) === false ) {
			return 0;
		}
